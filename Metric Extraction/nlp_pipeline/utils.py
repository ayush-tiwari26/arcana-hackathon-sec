...
def run_extractor(INPUT_FILINGS_PATH, OUTPUT_FILINGS_PATH, paragraph_extractor):
    ...
    for file in tqdm(files):
        ...
        data = {
            ...
            'date'     : [clean_date(r['date'], r['year']) for r in res],
...
def clean_date(date, year):
    ...
    """
    #TODO: Can Impliment Edit Distances Algorithm to define word similarity in O(n)
    if(date.strip()=='the prior year'):
        return str(year-1)
    cleaned_date = str(dateparser.parse(date).date())
